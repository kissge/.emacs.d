...
(el-get-bundle! git-gutter+ in nonsequitur/git-gutter-plus
  ...
  (define-key git-gutter+-mode-map (kbd "C-x /") 'git-gutter+-show-hunk-inline-at-point)
  (eval-when-compile (require 'diff-mode) (require 'cl-macs)) ;; todo:
  (define-key git-gutter+-mode-map (kbd "C-x ?")
    (defun git-gutter+-show-and-refine-hunk ()
      (interactive)
      (cl-letf ((diff-mode-orig (symbol-function 'diff-mode))
                ((symbol-function 'diff-mode)
                 (lambda () (funcall diff-mode-orig) (diff-refine-hunk))))
